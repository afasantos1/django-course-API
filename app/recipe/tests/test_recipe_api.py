...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_delete_recipe(self):
        """Test deleting a recipe successfil."""
        recipe = create_recipe(user=self.user)
    ...
    def test_create_recipe_with_new_ingredients(self):
        ...
        payload = {
            ...
            'time_minutes': 20,
            'price': Decimal('2.40'),
            'ingredients': [{'name': 'Sopa'}, {'name': 'De'}, {'name': 'Tomate'}]
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        payload = {'ingredients': [{'name': 'Pasta'}]}
        url = detail_url(recipe.id)
    ...
    def test_filter_by_tags(self):
        r1 = create_recipe(user=self.user, title='Fish and Chips')
        r2 = create_recipe(user=self.user, title='Carbonara')
        tag1 = Tag.objects.create(user=self.user, name='Fish')
        tag2 = Tag.objects.create(user=self.user, name='Meat')
        r3 = create_recipe(user=self.user, title='Mushroom Soup')
        r1.tags.add(tag1)
        r2.tags.add(tag2)
    ...
    def test_filter_by_ingredients(self):
        r1 = create_recipe(user=self.user, title='Fish and Chips')
        r2 = create_recipe(user=self.user, title='Carbonara')
        i1 = Ingredient.objects.create(user=self.user, name='Chips')
        i2 = Ingredient.objects.create(user=self.user, name='Bacon')
        r3 = create_recipe(user=self.user, title='Mushroom Soup')
